...
- **Plugin system — panels, sidebar, MCP, backend, chat & events** — Registry-driven plugin architecture lets any panel type, sidebar entry, MCP tool, backend hook, or chat event handler be contributed without touching Grove core. Plugins mount via a shared registry; skills can be authored, installed, and iterated directly from the chat surface.
- **PWA support** — Service worker with `no-store` cache strategy, maskable Android adaptive icon, CF-Access-compatible manifest link, and `Cache-Control: no-cache` on `sw.js` so browsers always fetch the latest worker without stale-cache surprises.
- **Deep link handling, Studio nav shortcut, compact dashboard UI** — Opens Grove directly to the right project from an OS-level URL; adds a Studio navigation keyboard shortcut; compacts the dashboard layout; refactors hooks for code reuse.
- **Adaptive tray / app icons** — Icons switch automatically with the system light/dark mode; macOS mobile header is alignment-corrected.
- **Smart git URL parsing** — Strips web-UI path fragments (`/tree/branch`, `/blob/branch/file`) from pasted GitHub/GitLab URLs so the project dialog accepts browser URLs without manual trimming.
- **Local skill management** — AI agents can author, install, and iterate on skills stored in `~/.claude/commands/` without leaving the chat session.

### Improved

- **GUI-only handlers feature-gated** — HTTP handlers that only make sense in a GUI build are now guarded with `#[cfg(feature = "gui")]`, eliminating dead-code warnings in non-GUI (TUI/CLI) binaries.
- **Service worker and Cache-Control consolidated** — SW fetch handler simplified to a single pass-through; `Cache-Control` headers unified in the axum middleware layer for consistency.

### Fixed

- **Sibling-chat `@` mentions sorted by recency** — The mention dropdown now orders sibling sessions newest-first instead of insertion order.
- **Tasks page hotkeys released when hidden** — Scope/context keys (j/k navigation, etc.) are properly released when `TasksPage` unmounts, preventing keystrokes from bleeding into other views.
- **Deleted-then-recreated file shows as untracked** — A file that was deleted and then recreated now appears as untracked in the git status panel rather than disappearing entirely.
- **Code Review: search refocuses on re-trigger; diff flex padding** — The code-search input correctly refocuses when retriggered; the diff panel flex layout no longer collapses its padding unexpectedly.
- **macOS IME `Shift+punctuation` bridged to PTY** — Sequences like `Shift+;` → `:` generated by the macOS IME now reach the terminal PTY correctly instead of being swallowed.
- **Hotkey `Mod`/`Shift` matching is now exact** — Bare letter shortcuts no longer accidentally fire for key combos that require an explicit modifier.
- **Code Review virtual files no longer leak into Changes mode** — Comment-based virtual file entries are filtered out of the Changes file list where they don't belong.
- **`NewTaskDialog` branch dropdown no longer clipped** — Removed `overflow-hidden` that was cutting off the branch picker popover.
- **Enter key restored after ESC from Workspace** — Pressing ESC in the Workspace view and then returning to the task list left the Enter key non-functional; fixed.
- **macOS notification permission banner hardened** — Edge cases in the permission-request flow that could leave the banner in a broken state are resolved.
- **Hover-scroll loop in command palettes fixed** — Hovering over palette items no longer triggers a feedback loop that caused unintended scrolling.
- **WAF `_sm_*` params stripped before HMAC validation** — WAF-injected query parameters starting with `_sm_` are now ignored during mobile HMAC signature verification, preventing spurious auth failures behind certain CDNs/WAFs.
