...
/// unit-testable without going through `grove_dir()`. Delegates to the
/// `Option`-based `load_history_from_path` (upstream) with an explicit cap.
fn load_history_from_path_capped(path: &Path, max_bytes: u64) -> Vec<AcpUpdate> {
    load_history_from_path(path, Some(max_bytes))
}

/// `max_read_bytes = None` reads the whole file — the compaction path relies on
/// this so it never truncates old events; `Some(n)` caps the tail read at `n`.
