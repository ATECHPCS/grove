...
pub(crate) const OPENCODE_KEY: &str = "minimax";
...
struct ModelRemain {
    ...
    current_interval_remaining_percent: Option<f32>,
    // Weekly window
    ...
    current_weekly_remaining_percent: Option<f32>,
}
...
pub(crate) fn fetch_with_token(token: &str) -> Result<AgentUsage, String> {
    ...
    for entry in entries {
        ...
        // Interval window — prefer server-provided percent; fall back to computed
            ...
            .or_else(|| {
                match (
                    ...
                    (Some(total), Some(remaining)) if total > 0 => {
                        Some(clamp_percent((remaining as f32 / total as f32) * 100.0))
                    }
                    _ => None,
                }
            });
        if let Some(pct) = interval_pct {
            ...
            let extra_value = match (
                entry.current_interval_usage_count,
                entry.current_interval_total_count,
            ) {
                (Some(used), Some(total)) if total > 0 => {
                    format!("{} / {} left", total - used, total)
                }
                _ => format!("{}% remaining", pct as u32),
            };
        ...
        // Weekly window — prefer server-provided percent; fall back to computed
